...
router.post("/signup", async (req, res, next) => {
  ...
  await dbConnection(
    ...
    res,
    500,
    "Could not sign up user, an error occurred."
  );
});
...
router.post("/login", async (req, res, next) => {
  await dbConnection(
    async (conn) => {
      ...
      const userValid =
        response[0].length != 0 &&
        (await bcrypt.compare(req.body.password, response[0][0].password));
      ...
      sql =
        "SELECT id, name, email_address, role FROM users WHERE email_address = '" +
    ...
    res,
    500,
    "Could not login user, an error occured."
  );
});
