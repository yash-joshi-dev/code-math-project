...
async function updateLesson(conn, req, lessonId) {
  ...
  await conn.query(`UPDATE lessons SET ?`, updatedLessonData);
}

//get lesson FUNCTION (get all definitions for the lesson with it)
async function getLesson(conn, req, lessonId) {
  const lessonData = (await conn.query(`SELECT * FROM lessons WHERE lesson_id = ${lessonId}`))[0][0];
  const definitionsMapping = lessonData.definitions_mapping.join(", ");
  const definitions = (await conn.query(`SELECT * FROM definitions WHERE id IN (${lessonData.definitions_mapping}) ORDER BY FIELD (id, ${definitionsMapping})`))[0];
